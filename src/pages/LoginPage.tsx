  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      const loginResult = await Promise.race([
        ...
        new Promise<never>((_, reject) ***REMOVED*** {
          setTimeout(() ***REMOVED*** reject(new Error("timeout")), 12000);
        }),
      ]);
    ...
    <main className="mx-auto flex min-h-screen max-w-md items-center px-4">
      <form onSubmit={handleSubmit} className="w-full space-y-4 rounded-xl bg-white p-6 shadow">
        ...
        <label className="block">
          ...
          <input className="w-full rounded border px-3 py-2" type="password" value={password} onChange={(e) ***REMOVED*** setPassword(e.target.value)} required />
        </label>
        ...
        <button className="w-full rounded border px-4 py-2 text-sm" onClick={() ***REMOVED*** void handleResetLocalData()} type="button">
