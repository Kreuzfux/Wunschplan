...
export function ProfilePage() {
  ...
  useEffect(() ***REMOVED*** {
    async function loadLimit() {
      ...
      if (typeof value ***REMOVED*** "number") setMaxShifts(String(value));
    }
    void loadLimit();
  }, [profile?.id]);
  ...
  const currentAvatar = useMemo(() ***REMOVED*** profile?.avatar_url ?? null, [profile?.avatar_url]);
        ...
        <div className="mt-3 space-y-3 text-sm">
          ...
          <label className="block">
            ...
            <input
              ...
              max={366}
              value={maxShifts}
              onChange={(e) ***REMOVED*** setMaxShifts(e.target.value)}
              disabled={busy}
            />
          ...
          <button
            ...
            disabled={busy}
            onClick={() ***REMOVED*** void saveProfile()}
          >
            Speichern
          </button>
        </div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-4">
          ...
          <div className="text-sm">
            ...
            <div className="mt-2 flex gap-2">
              <button className="rounded border px-3 py-1 disabled:opacity-60" disabled={busy || !avatarFile} onClick={() ***REMOVED*** void uploadAvatar()}>
                Hochladen
              </button>
