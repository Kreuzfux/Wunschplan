...
export function ChatPage() {
  ...
  const activeThreadId = tab ***REMOVED*** "team" ? teamThreadId : dmThreadId;
  ...
  const canSend = useMemo(() ***REMOVED*** {
    ...
    if (tab ***REMOVED*** "dm" && !dmThreadId) return false;
  ...
  useEffect(() ***REMOVED*** {
    async function loadDmUsers() {
      ...
      const { data } = await supabase
        ...
        .eq("is_active", true)
        .order("full_name");
      const options = ((data ?? []) as DmUserOption[]).filter((u) ***REMOVED*** u.id !== profile.id);
      setDmUsers(options);
    }
    loadDmUsers();
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between gap-3 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex gap-2">
            <button
              className={`px-3 py-1 rounded border ${tab ***REMOVED*** "team" ? "bg-gray-900 text-white" : "bg-white"}`}
              onClick={() ***REMOVED*** setTab("team")}
            >
              Teamchat
            </button>
            <button
              className={`px-3 py-1 rounded border ${tab ***REMOVED*** "dm" ? "bg-gray-900 text-white" : "bg-white"}`}
              onClick={() ***REMOVED*** setTab("dm")}
            >
              Direkt
            </button>
          </div>
      ...
      {tab ***REMOVED*** "dm" ? (
        <div className="mb-4 flex flex-col md:flex-row gap-2 md:items-center">
          <select
            ...
            onChange={(e) ***REMOVED*** setSelectedDmUserId(e.target.value)}
          >
            <option value="">Person auswählen…</option>
            {dmUsers.map((u) ***REMOVED*** (
          ...
          <button
            ...
            onClick={() ***REMOVED*** ensureDmThread(selectedDmUserId)}
          >
            DM öffnen
          </button>
        </div>
      ...
      {fullscreenImage ? (
        <div className="fixed inset-0 z-50 bg-black/80 p-4 flex items-center justify-center" onClick={() ***REMOVED*** setFullscreenImage(null)}>
          <div className="relative max-w-[95vw] max-h-[95vh] flex flex-col items-center gap-3" onClick={(e) ***REMOVED*** e.stopPropagation()}>
            <button
            ...
            <button className="rounded bg-white px-4 py-2 text-sm hover:bg-gray-100" onClick={() ***REMOVED*** downloadAttachment(fullscreenImage.attachment)}>
              Download
            </button>
          </div>
        </div>
